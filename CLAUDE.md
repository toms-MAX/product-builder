...
현재는 중·고등학교 내신/문법 문제 중심이지만,
같은 DB 구조와 템플릿 엔진으로 아래 시장까지 확장 가능하다.

### 확장 로드맵

| 단계 | 타겟 시장 | 핵심 추가 요소 |
|---|---|---|
| **Phase 1 (현재)** | 중·고등학교 영어 내신 | 문법 문제, 단어 DB, 시험지 PDF |
| **Phase 2** | 수능 영어 | 빈칸추론·순서배열·삽입 등 수능 유형 추가 |
| **Phase 3** | 토익 (TOEIC) | Part 5/6 문법 문제, 비즈니스 단어 DB |
| **Phase 4** | 아이엘츠 (IELTS) | 아카데믹 어휘, Writing Task 문법 드릴 |
| **Phase 5** | 공무원 영어 / 편입 | 고난도 어휘, 독해 문법 문제 유형 |
...
문법 (메인 트랙)                독해 (서브 트랙)
─────────────────────           ──────────────────────────
지금 당장 DB + 템플릿 구축       지문 DB 테이블 구조만 설계
저작권 리스크 없음               AI 원본 지문 생성 소규모 실험
구독자 확보 → 현금흐름           품질 검수 기준 정립
                                → 구독자 200명 달성 후 정식 추가
```

**독해 지문 저작권 원칙**
- 교과서·수능·토익 기출 지문 직접 사용 금지 (저작권 위반)
- 허용 경로 3가지:
  1. AI가 주제+난이도+핵심어휘 조건으로 원본 생성 (주 방향)
  2. Wikipedia 등 공개 도메인 텍스트 활용
  3. 강사가 직접 작성한 지문 입력
- 지문 생성은 "AI가 원본 제작"이 예외적으로 허용되는 영역
  (프로젝트 철학 "AI는 선택만"의 공식 예외 케이스)

### 확장이 쉬운 이유 (구조적 강점)

```
현재 구조에서 시험 유형을 추가하려면:
1. words 테이블에 해당 시험 단어 DB만 추가
2. templates 테이블에 해당 시험 유형 템플릿만 추가
3. level 필드에 "TOEIC" / "IELTS" 등 값 추가

→ 핵심 엔진(슬롯 시스템, 문제 생성 로직)은 그대로 재사용
→ 데이터가 자산인 구조이기 때문에 확장 비용이 매우 낮다
```
...
| 플랜 | 대상 | 가격 |
|---|---|---|
| 기본 (현재) | 학원 강사 | 월 9,900원 |
| 수험생 개인 | 토익·아이엘츠 준비생 | 월 4,900원 |
| 학원 팀 | 강사 5인 이상 | 월 39,000원 |
| 기관 | 학원·학교 단위 | 별도 협의 |
